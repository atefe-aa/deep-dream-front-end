...
import { FilterState } from "../../../app/ui/search-and-filter/_models";
...
function stringifyFilterChartQuery(state: FilterState): string {
  const fromDate = "fromDate="+state.fromDate;
  const toDate ="toDate="+ state.toDate;
  const testTypes = isNotEmpty(state.testTypes)
    ? state.testTypes.map((test) => `testTypes[]=${test}`).join("&")
    : "";
  const laboratories = isNotEmpty(state.laboratories)
    ? state.laboratories.map((lab) => `laboratories[]=${lab}`).join("&")
    : "";

  return [fromDate, toDate, testTypes, laboratories]
    .filter((f) => f)
    .join("&");
}
